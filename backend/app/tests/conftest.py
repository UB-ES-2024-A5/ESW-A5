...
from app.models import User, Account, Link, Book, WishList, WishlistBookLink, Follower, Review, Comment, Forum, ForumReaction
...
def db() -> Generator[Session, None, None]:
    with Session(engine) as session:
        try:
            ...
            session.exec(delete(ForumReaction))
            session.exec(delete(Forum))
            session.exec(delete(Link))
            session.exec(delete(Book))
            session.exec(delete(Follower))
